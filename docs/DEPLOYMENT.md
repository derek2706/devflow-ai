...
| `AI_MODE`             | `provider` for real AI, or `local` for deterministic planning                                                       |
| `AI_PROVIDER`         | `groq` for the free API-key option                                                                                  |
| `GROQ_API_KEY`        | Private key from your Groq Free account                                                                             |
...
Use **Groq Free** with the variables above. Create a key in the [Groq console](https://console.groq.com/keys) and enter it privately as `GROQ_API_KEY` in Vercel Production settings. Keep the Free plan; a paid upgrade is not required. Redeploy after saving the variables. Do not paste the key into source files or browser configuration.

For `openai/gpt-oss-120b`, Groq currently lists 30 requests/minute, 1,000 requests/day, 8,000 tokens/minute, and 200,000 tokens/day per organization. All limits apply, so token limits can be reached first. Check the account's actual allowance and [current rate-limit documentation](https://console.groq.com/docs/rate-limits). The app uses its [Responses API](https://console.groq.com/docs/responses-api) and validates the resulting JSON.

After deploying with Groq enabled, sign in and use **AI summary** or **Plan a sprint** on a project, **Generate subtasks** on a saved task, or **Generate standup** on the dashboard. Generate a draft and check the engine shown in the panel. Review suggested subtasks before adding them to the board. Confirming `/api/ai/status` reports provider mode is a configuration check, not proof that a model request succeeded.

The server uses native `fetch` to `https://api.groq.com/openai/v1/responses` with strict JSON schemas and `store: false`. It validates each draft and caps a request at 30 seconds and 3,000 output tokens. When you click Generate in provider mode, it sends a limited task snapshot to Groq, excluding credentials, member emails, and comments. A notice appears if the external context was shortened. Quota/rate limits, timeouts, temporary service failures, or invalid model output produce a clearly labeled local draft and reason. Missing credentials, Groq HTTP 401/403 responses, rejected model/request settings, and application authorization or database errors remain errors without a fallback. No automatic retries or alternate models are used.

To run without external generation, set `AI_MODE=local`; this is the example environment's default. Previous `OPENAI_API_KEY` / `OPENAI_MODEL` settings are ignored. Replace an old `AI_PROVIDER` value with `groq` or remove it to use the default. Keep credentials server-only and redeploy after changing variables. Stay on Groq Free instead of enabling a paid plan to resolve a quota limit.

### Verify generation and fallbacks

1. Run `pnpm test` before deploying. The AI tests use mocked HTTP responses, so failures and quotas can be exercised without sending task data or consuming the free allowance. Check coverage for valid output, HTTP 429, timeout, unavailable service, invalid JSON/schema, invalid sprint task IDs, and credential/configuration errors that must not fall back.
2. After deploying, use a small disposable project to generate a subtask draft, project summary, sprint plan, and standup. A real result displays **Generated by Groq**. Its API response must contain `data.mode: "provider"`, `provider: "groq"`, the configured `model`, and no `fallbackReason`.
3. For a simulated recoverable upstream failure in the local test suite, expect `mode: "local"` with the attempted provider/model and a reason of `quota`, `timeout`, `unavailable`, or `invalid_response`. The UI should identify the result as a local draft. A mocked Groq HTTP 401/403 must instead return a safe API error with no draft. Use mocks rather than deliberately exhausting production quota or replacing a valid production key.
4. For a shortened snapshot, check `contextLimited: true` and the notice on the Groq result. Confirm that suggested sprint tasks belong to the project and that subtask suggestions create board tasks only after you explicitly add them.

Record successful live checks only after these result fields and labels are verified. The status endpoint and an HTTP 200 fallback alone do not establish real AI delivery.

## Build and verification

The local regression suite currently passes 168 tests: 114 backend, 39 web API/bridge/render tests, 9 migration guards, and 6 production configuration guards. This includes 43 focused AI tests with mocked provider responses. The web tests cover startup diagnostics, cookie preservation, concurrent session refresh, task form values, board filters/permissions, and AI provenance rendering. The separate local database suite passed 10 checks, including immediate session revocation and single-query session validation. Local test results and a successful build do not establish the hosted API's health; complete the live checks below after each release.
...
- Generate one planning draft and verify its engine label. A local fallback confirms the fallback works, not that live AI succeeded.
...
- The signed-in shell persists across navigation. Loading placeholders stay within the changing content, and account changes clear the prior user's workspace state.

## Demo limits

- Real AI uses Groq's limited allowance. Quota and temporary service failures produce labeled local drafts; credential/configuration errors need a fix. Local mode makes no external AI requests.
...
- **AI shows a local draft:** read the reason in the panel. For quota/rate limits, wait and check the provider's allowance; do not enable paid upgrades or top-ups to fix a free-tier limit.
- **AI configuration error:** verify `AI_MODE=provider`, `AI_PROVIDER=groq`, the documented model ID, and a private `GROQ_API_KEY`. Groq HTTP 401/403 responses become safe API 503 errors without a local draft. Correct the key/account access and create a fresh deployment after changing variables.
